...
public:
    ...
    void backward(double hat_Y, double Y)
    {
        ...
        //////////////////////////////////////////////////////////////////////////////////////////
        double aLt_aWx[nStates][nUnits] = {
            0,
        };
        double aLt_aBx[nUnits] = {
            0,
        };
        ...
        for (int i = 0; i < nUnits; ++i)
        {
            ...
            {
                aLt_aWx[j][i] = (hat_Y - Y) * Wy[i] * (1 - H[0][i]) * (1 + H[0][i]) * Xmem[0][j] + (hat_Y - Y) * Wy[i] * (1 - H[0][i]) * (1 + H[0][i]) * sum_Wh * (1 - H[1][i]) * (1 + H[1][i]) * Xmem[1][j] + (hat_Y - Y) * Wy[i] * (1 - H[0][i]) * (1 + H[0][i]) * sum_Wh * (1 - H[1][i]) * (1 + H[1][i]) * sum_Wh * (1 - H[2][i]) * (1 + H[2][i]) * Xmem[2][j];

                Wx[j][i] = Wx[j][i] - lr * aLt_aWx[j][i];
            }
            aLt_aBx[i] = (hat_Y - Y) * Wy[i] * (1 - H[0][i]) * (1 + H[0][i]) * 1 + (hat_Y - Y) * Wy[i] * (1 - H[0][i]) * (1 + H[0][i]) * sum_Wh * (1 - H[1][i]) * (1 + H[1][i]) * 1 + (hat_Y - Y) * Wy[i] * (1 - H[0][i]) * (1 + H[0][i]) * sum_Wh * (1 - H[1][i]) * (1 + H[1][i]) * sum_Wh * (1 - H[2][i]) * (1 + H[2][i]) * 1;
        ...
        for (int i = 0; i < nUnits; ++i)
        {
            ...
            for (int j = 0; j < nUnits; ++j)
            {
                aLt_aWh[i][j] = (hat_Y - Y) * Wy[i] * (1 - H[0][i]) * (1 + H[0][i]) * H[1][j] + (hat_Y - Y) * Wy[i] * (1 - H[0][i]) * (1 + H[0][i]) * sum_Wh * (1 - H[1][i]) * (1 + H[1][i]) * H[2][j] + (hat_Y - Y) * Wy[i] * (1 - H[0][i]) * (1 + H[0][i]) * sum_Wh * (1 - H[1][i]) * (1 + H[1][i]) * sum_Wh * (1 - H[2][i]) * (1 + H[2][i]) * H[3][j];
...
int main()
{
    ...
    // pre-processing
    preprocessing::normalized(pos, 5); // Increased predict performance after 'pos' normalized and scaled similar to 'acc' (23.4/5)
